...
func TestTranspileSimpleJoin(t *testing.T) {
	...
	// FROM and JOIN name the relations. Each table reference is one address.
	addresses := []string{iamUsers, iamAttachedPolicies}
	...
	// The SELECT list becomes one projection per address. A computed column is a function call; a
	...
	usersSelect, err := omnisdk.NewProjection(iamUsers, []omnisdk.SelectColumn{
	...
	g, err := omnisdk.NewGraphWithProjections(addresses, []omnisdk.Wiring{join},
